...
const requestContext = require('./requestContext');
const customFormat = format.combine(
  format.timestamp({ format: 'YYYY-MM-DD hh:mm:ss.SSS A ZZ' }),
  format((info) => {
    const req = requestContext.getRequest();
    if (req && req.path) {
      info.route = req.originalUrl || req.path;
       info.method = req.method;
       info.body =  req.body? req.body: 'not body data exsist'
       info.prams =  req.params? req.params: 'not params data exsist'
       info.query =  req.query? req.query: 'not query data exsist'
    }
    return info;
  })(),
  format.json()
